...
title: Delete user
excerpt: >-
  Deleting a user refers to removing an account from the system when an
  individual no longer requires access, such as when they leave the organisation
  or change positions.
deprecated: false
hidden: false
...
next:
  description: ''
---
In SAPIENT, you can delete the users when they are no longer required to reduce potential security risks by ensuring that only authorised personnel have access to sensitive shipping information and operations.

Additionally, this process helps streamline user management by removing inactive or unnecessary accounts, thereby improving system performance and clarity.

<br />

<ToggleList>
  To delete a user in SAPIENT, perform the steps as explained in the following procedure:
