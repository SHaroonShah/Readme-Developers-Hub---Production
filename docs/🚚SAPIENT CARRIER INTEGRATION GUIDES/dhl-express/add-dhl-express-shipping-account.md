...
| Element | Description |
| --- | --- |
| **Carrier Account Number**\* | Enter the carrier account number assigned to you by DHL Express. |
...
| Element | Description |
| --- | --- |
| **API Key**\* | Enter the API key for the user account associated with DHL Express. |
| **API Secret**\* | Enter the API secret used to authenticate the user account with DHL Express. |
| **Archive Labels** | Turn on the toggle if you wish to include the DHL archived labels in the manifest.<br /><br />Enabling this toggle saves DHL waybill documents from the create shipment response in the database and includes them in the manifest PDF when shipments are manifested.Note: The manifests can be downloaded from the SAPIENT’s View manifest history page.<br /><br />`Note`: *The manifests can be downloaded from the SAPIENT’s [View manifest history page](https://docs.intersoftsapient.net/docs/manifest-history).* |
| **Enable Paperless Trade (PLT)** | Turn on the toggle if you want to submit the customs documents electronically. If this toggle is enabled, then you must specify one the following options: |
| **Allow DHL to generate commercial invoices on your behalf** | Select this option for DHL to create and send the invoice electronically to the customs agency. |
| **Use SAPIENT-generated invoices** | Select this option to have SAPIENT generate the invoices from shipment data and send them to DHL. |
