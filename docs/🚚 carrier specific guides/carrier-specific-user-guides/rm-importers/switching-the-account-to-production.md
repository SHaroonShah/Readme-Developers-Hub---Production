...
* **Royal Mail importers using Freight 2 Post route**: involves CDS checks as well as physical label sign off. For\
  CDS checks, you are requested to create a couple of <Glossary>labels</Glossary>, <Glossary>manifest</Glossary> them and share with us the <Glossary>PDF</Glossary>s, so they can be checked in first as part of the account set up process. 
