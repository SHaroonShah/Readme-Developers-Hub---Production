		...
		public class ManifestPickedShipments
    {
        public async Task RunTheCodeAsync()
        {
            ...
            var shipmentResponse = await client.PostAsync<ShipmentResponse>(shipmentRequest);
						...
						// Update shipments status to 'Picked'
						var updateStatusRequest = new RestRequest($"/v4/shipments/status", Method.Put);

            updateStatusRequest.AddJsonBody(new
            {
                Status = "Picked",
                ShipmentIds = new[]
                {
                    shipmentId
						...
						await client.ExecuteAsync(updateStatusRequest);
...
# Update the shipment status to "Picked"

<!-- csharp@93-105 -->

Update the shipment status to Picked.

# Manifest Picked shipment
